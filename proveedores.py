# proveedores.py
import flet as ft
from cliente import connect_to_db
...
class Herramienta_Proveedor:
    ...
    def mostrar_proveedores(self):
        self.page.clean()
        self.cursor.execute("SELECT id, nombre, email, telefono, direccion FROM proveedores ORDER BY nombre")
        ...
        for p in datos:
            ...
            rows.append(ft.DataRow(cells=[ft.DataCell(ft.Text(p[1])), ft.DataCell(ft.Text(p[2])),
                                        ft.DataCell(ft.Text(p[3])), ft.DataCell(ft.Text(p[4])),
        ...
        self.page.add(ft.Column([header, ft.DataTable(
            columns=[ft.DataColumn(ft.Text("Nombre")), ft.DataColumn(ft.Text("Email")),
                    ft.DataColumn(ft.Text("Teléfono")), ft.DataColumn(ft.Text("Dirección")),
